...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const url = `${backendUrl.replace(/\/$/, "")}/strategy/feedback/history?user_id=${userId}&limit=${limit}`
    ...
    return NextResponse.json({ error: "strategy_feedback_history_request_failed" }, { status: 500 })
  }
}
